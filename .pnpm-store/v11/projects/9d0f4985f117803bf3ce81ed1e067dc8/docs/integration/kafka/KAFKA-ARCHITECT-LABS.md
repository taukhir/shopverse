...
```mermaid
flowchart LR
    H["State a delivery hypothesis"] --> C["Configure producer, topic, and consumer"] --> F["Inject load or failure"]
    F --> O["Observe lag, retries, ordering, and DLT"] --> E["Explain the trade-off"] --> H
```
