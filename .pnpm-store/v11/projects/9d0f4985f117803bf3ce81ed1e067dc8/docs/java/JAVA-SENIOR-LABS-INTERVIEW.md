...
## Senior Java Investigation Loop

```mermaid
flowchart LR
    S["Reproduce the symptom"] --> E["Collect JVM evidence"] --> H["Form a runtime hypothesis"]
    H --> X["Run a controlled experiment"] --> M["Measure behavior and trade-offs"] --> S
```
