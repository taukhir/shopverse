...
```mermaid
flowchart LR
    Evidence["Observed answer evidence"] --> Score["Score against anchored rubric"] --> Gap["Identify the dominant gap"]
    Gap --> Route["Route to focused learning pages"] --> Practice["Assign a measurable exercise"] --> Reassess
```
