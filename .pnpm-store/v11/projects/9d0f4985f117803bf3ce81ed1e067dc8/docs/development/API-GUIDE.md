...
## Shopverse API Request Path

```mermaid
flowchart LR
    Client -->|"JWT and correlation ID"| Gateway --> Service
    Service -->|"authorize ownership"| Domain["Domain operation"]
    Domain --> Database
    Domain -->|"integration intent"| Outbox --> Kafka
```
