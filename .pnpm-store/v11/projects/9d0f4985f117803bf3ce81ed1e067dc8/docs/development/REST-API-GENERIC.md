...
```mermaid
flowchart LR
    Request --> Authenticate --> Authorize --> Validate --> Execute --> Represent --> Respond
    Execute --> Error["Domain or infrastructure error"] --> Problem["Consistent problem response"] --> Respond
```
