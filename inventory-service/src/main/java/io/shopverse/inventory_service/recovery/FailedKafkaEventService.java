...
public class FailedKafkaEventService {
    ...
    public FailedKafkaEventResponse replay(Long id, String username) {
        try {
            return toResponse(recoveryService.replay(id, username));
    ...
    private FailedKafkaEventResponse toResponse(FailedKafkaEventRecord event) {
        return new FailedKafkaEventResponse(
                event.id(), event.sourceTopic(), event.payload(), event.failureReason(),
                event.retryCount(), event.replayed(), event.replayCount(),
                event.lastReplayedBy(), event.failedAt(), event.replayedAt());
    }
}
