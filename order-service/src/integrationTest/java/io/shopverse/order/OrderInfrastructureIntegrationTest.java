...
class OrderInfrastructureIntegrationTest {
    ...
    @Test
    void migrationsCreateOrderAndRecoveryTables() {
    ...
    void kafkaBrokerAcceptsAnOutboxPayload() throws Exception {
        String topic = "shopverse.test.order." + UUID.randomUUID();
        var result = kafkaTemplate.send(topic, "order-key", "{\"status\":\"test\"}")
