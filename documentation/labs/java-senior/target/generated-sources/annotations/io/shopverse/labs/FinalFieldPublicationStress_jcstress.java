...
import io.shopverse.labs.FinalFieldPublicationStress;
import org.openjdk.jcstress.infra.results.I_Result;

public final class FinalFieldPublicationStress_jcstress extends Runner<I_Result> {

    volatile WorkerSync workerSync;

    public FinalFieldPublicationStress_jcstress(ForkedTestConfig config) {
        super(config);
    }
    ...
    private static class JcstressThread_APICheck_writer extends VoidThread {
        FinalFieldPublicationStress t;
        FinalFieldPublicationStress s;
        I_Result r;

        public JcstressThread_APICheck_writer(FinalFieldPublicationStress t, FinalFieldPublicationStress s, I_Result r) {
    ...
    private static class JcstressThread_APICheck_reader extends VoidThread {
        ...
        public JcstressThread_APICheck_reader(FinalFieldPublicationStress t, FinalFieldPublicationStress s, I_Result r) {
    ...
    private void jcstress_sanityCheck_API(Counter<I_Result> counter) throws Throwable {
        final FinalFieldPublicationStress s = new FinalFieldPublicationStress();
    ...
    private static class JcstressThread_ResourceCheck_writer extends LongThread {
        FinalFieldPublicationStress[] ss;
        I_Result[] rs;
        int size;

        public JcstressThread_ResourceCheck_writer(FinalFieldPublicationStress[] ss, I_Result[] rs, int size) {
        ...
        private void jcstress_check_writer(FinalFieldPublicationStress[] ls, I_Result[] lr, int size) {
    ...
    private static class JcstressThread_ResourceCheck_reader extends LongThread {
        ...
        public JcstressThread_ResourceCheck_reader(FinalFieldPublicationStress[] ss, I_Result[] rs, int size) {
        ...
        private void jcstress_check_reader(FinalFieldPublicationStress[] ls, I_Result[] lr, int size) {
            for (int c = 0; c < size; c++) {
                ls[c].reader(lr[c]);
            }
        }
    ...
    private static class TestResourceEstimator implements ResourceEstimator {
        ...
        public void runWith(int size, long[] cnts) {
            ...
            FinalFieldPublicationStress[] ls = new FinalFieldPublicationStress[size];
            I_Result[] lr = new I_Result[size];
            for (int c = 0; c < size; c++) {
                FinalFieldPublicationStress s = new FinalFieldPublicationStress();
    ...
    public ArrayList<CounterThread<I_Result>> internalRun() {
        ...
        FinalFieldPublicationStress[] ls = new FinalFieldPublicationStress[len];
        I_Result[] lr = new I_Result[len];
        for (int c = 0; c < len; c++) {
            ls[c] = new FinalFieldPublicationStress();
            lr[c] = new I_Result();
        }
    ...
    public static void jcstress_ni_consume_final(Counter<I_Result> cnt, FinalFieldPublicationStress[] ls, I_Result[] lr, FinalFieldPublicationStress test, int len, int a) {
        int left = a * len / 2;
        int right = (a + 1) * len / 2;
        for (int c = left; c < right; c++) {
            I_Result r = lr[c];
            FinalFieldPublicationStress s = ls[c];
            cnt.record(r, 1);
        }
    }

    public static void jcstress_consume_reinit(Counter<I_Result> cnt, FinalFieldPublicationStress[] ls, I_Result[] lr, FinalFieldPublicationStress test, int len, int a) {
        int left = a * len / 2;
        int right = (a + 1) * len / 2;
        for (int c = left; c < right; c++) {
            I_Result r = lr[c];
    ...
    public class JcstressThread_writer extends CounterThread<I_Result> {
        FinalFieldPublicationStress[] ss;
        I_Result[] rs;
        FinalFieldPublicationStress test;

        public JcstressThread_writer(FinalFieldPublicationStress[] ss, I_Result[] rs, FinalFieldPublicationStress test) {
        ...
        private void jcstress_stride_writer(int start, int end) {
            FinalFieldPublicationStress[] ls = ss;
            I_Result[] lr = rs;
    ...
    public class JcstressThread_reader extends CounterThread<I_Result> {
        ...
        public JcstressThread_reader(FinalFieldPublicationStress[] ss, I_Result[] rs, FinalFieldPublicationStress test) {
