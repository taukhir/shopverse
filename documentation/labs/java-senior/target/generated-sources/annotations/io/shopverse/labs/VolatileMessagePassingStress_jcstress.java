...
import io.shopverse.labs.VolatileMessagePassingStress;
import org.openjdk.jcstress.infra.results.I_Result;

public final class VolatileMessagePassingStress_jcstress extends Runner<I_Result> {

    volatile WorkerSync workerSync;

    public VolatileMessagePassingStress_jcstress(ForkedTestConfig config) {
        super(config);
    }
    ...
    private static class JcstressThread_APICheck_writer extends VoidThread {
        VolatileMessagePassingStress t;
        VolatileMessagePassingStress s;
        I_Result r;

        public JcstressThread_APICheck_writer(VolatileMessagePassingStress t, VolatileMessagePassingStress s, I_Result r) {
    ...
    private static class JcstressThread_APICheck_reader extends VoidThread {
        ...
        public JcstressThread_APICheck_reader(VolatileMessagePassingStress t, VolatileMessagePassingStress s, I_Result r) {
    ...
    private void jcstress_sanityCheck_API(Counter<I_Result> counter) throws Throwable {
        final VolatileMessagePassingStress s = new VolatileMessagePassingStress();
    ...
    private static class JcstressThread_ResourceCheck_writer extends LongThread {
        VolatileMessagePassingStress[] ss;
        I_Result[] rs;
        int size;

        public JcstressThread_ResourceCheck_writer(VolatileMessagePassingStress[] ss, I_Result[] rs, int size) {
        ...
        private void jcstress_check_writer(VolatileMessagePassingStress[] ls, I_Result[] lr, int size) {
    ...
    private static class JcstressThread_ResourceCheck_reader extends LongThread {
        ...
        public JcstressThread_ResourceCheck_reader(VolatileMessagePassingStress[] ss, I_Result[] rs, int size) {
        ...
        private void jcstress_check_reader(VolatileMessagePassingStress[] ls, I_Result[] lr, int size) {
            for (int c = 0; c < size; c++) {
                ls[c].reader(lr[c]);
            }
        }
    ...
    private static class TestResourceEstimator implements ResourceEstimator {
        ...
        public void runWith(int size, long[] cnts) {
            ...
            VolatileMessagePassingStress[] ls = new VolatileMessagePassingStress[size];
            I_Result[] lr = new I_Result[size];
            for (int c = 0; c < size; c++) {
                VolatileMessagePassingStress s = new VolatileMessagePassingStress();
    ...
    public ArrayList<CounterThread<I_Result>> internalRun() {
        ...
        VolatileMessagePassingStress[] ls = new VolatileMessagePassingStress[len];
        I_Result[] lr = new I_Result[len];
        for (int c = 0; c < len; c++) {
            ls[c] = new VolatileMessagePassingStress();
            lr[c] = new I_Result();
        }
    ...
    public static void jcstress_ni_consume_final(Counter<I_Result> cnt, VolatileMessagePassingStress[] ls, I_Result[] lr, VolatileMessagePassingStress test, int len, int a) {
        int left = a * len / 2;
        int right = (a + 1) * len / 2;
        for (int c = left; c < right; c++) {
            I_Result r = lr[c];
            VolatileMessagePassingStress s = ls[c];
            cnt.record(r, 1);
        }
    }

    public static void jcstress_consume_reinit(Counter<I_Result> cnt, VolatileMessagePassingStress[] ls, I_Result[] lr, VolatileMessagePassingStress test, int len, int a) {
        int left = a * len / 2;
        int right = (a + 1) * len / 2;
        for (int c = left; c < right; c++) {
            I_Result r = lr[c];
    ...
    public class JcstressThread_writer extends CounterThread<I_Result> {
        VolatileMessagePassingStress[] ss;
        I_Result[] rs;
        VolatileMessagePassingStress test;

        public JcstressThread_writer(VolatileMessagePassingStress[] ss, I_Result[] rs, VolatileMessagePassingStress test) {
        ...
        private void jcstress_stride_writer(int start, int end) {
            VolatileMessagePassingStress[] ls = ss;
            I_Result[] lr = rs;
    ...
    public class JcstressThread_reader extends CounterThread<I_Result> {
        ...
        public JcstressThread_reader(VolatileMessagePassingStress[] ss, I_Result[] rs, VolatileMessagePassingStress test) {
