# Spring And Kafka Transactions

A transaction groups operations into one atomic unit: either its changes
commit or they roll back. This guide explains reusable Spring, JPA, database,
and Kafka transaction behavior. The exact Shopverse boundaries are documented
in [Shopverse transactions](TRANSACTIONS.md).
...
Kafka idempotent production prevents duplicate writes caused by producer
retries within a producer session. Kafka transactions additionally group
multiple records and, for consume-process-produce pipelines, consumed offsets:
...
Consumers requiring committed data use `isolation.level=read_committed`.
Transactional producers require a unique transactional ID per producer
instance. Spring Boot can create transactional producer infrastructure when a
Kafka transaction ID prefix is configured.

Kafka exactly-once semantics apply within the Kafka transaction boundary. They
do not make a MySQL update and Kafka send one atomic commit.

## Database Plus Kafka

This is an unsafe dual write:
...
The database can commit while Kafka fails, or Kafka can accept a record before
the database rolls back. Use a transactional outbox:
...
A separate publisher sends durable outbox records. Delivery is normally at
least once, so consumers must be idempotent.
...
9. use outbox/SAGA for database and broker consistency.
...
- [SAGA and outbox patterns](SAGA-GENERIC.md)
