...
class PaymentInfrastructureIntegrationTest {
    ...
    @Test
    void migrationsCreatePaymentAndRecoveryTables() {
    ...
    void kafkaBrokerAcceptsAnOutboxPayload() throws Exception {
        String topic = "shopverse.test.payment." + UUID.randomUUID();
        var result = kafkaTemplate.send(topic, "payment-key", "{\"status\":\"test\"}")
